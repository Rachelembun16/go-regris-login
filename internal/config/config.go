...
type DatabaseConfig struct {
	...
	Password string `env:"DATABASE_PASSWORD,default=1234"`
	Name     string `env:"DATABASE_NAME,default=database_buku"`
}

// JWTConfig holds configuration for JWT secret key
type JWTConfig struct {
	SecretKey string `env:"JWT_SECRET_KEY,default=rahasia"`
}

// InternalConfig holds configuration for internal communication between microservices.
type InternalConfig struct {
	Username string `env:"SVC_USERNAME,default=schule"`
	Password string `env:"SVC_PASSWORD,default=leerling"`
}

// GCPConfig holds configuration for GCP.
type GCPConfig struct {
	ProjectID string `env:"GOOGLE_APPLICATION_PROJECT_ID"`
}
