...
func main() {
	...
	log.Info().Msg(" BukuModel [" + (&entity.Buku{}).TableName() + "]")
	db.AutoMigrate(&entity.Buku{})

	log.Info().Msg(" KategoriModel [" + (&entity.Kategori{}).TableName() + "]")
	db.AutoMigrate(&entity.Kategori{})

	log.Info().Msg("PeminjamanModel [" + (&entity.Peminjaman{}).TableName() + "]")
	db.AutoMigrate(&entity.Peminjaman{})
